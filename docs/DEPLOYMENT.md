...
## Redeploying an existing box

Once the instance in step 1 exists, a routine deploy is two commands from the repo root:

```bash
./scripts/deploy-backend.sh 3.110.135.118    # ship + rebuild + wait for health
./scripts/smoke-test.sh     3.110.135.118    # prove the things that fail quietly
```

`deploy-backend.sh` archives the **committed** tree with `git archive HEAD`, copies it up, runs
`docker compose up -d --build backend`, then polls `/api/v1/health` until it answers — dumping the
container logs and exiting non-zero if it never does. It refuses to run against a dirty working
tree, because otherwise the box silently lags your editor and nobody can say what is in production.

`--build` is not optional. Some fixes live in the `Dockerfile` itself (the PDF font assets, for
one), and `docker compose restart` reuses the existing image and changes nothing.

`backend/.env` is gitignored, so it is **not** in the archive and is never overwritten — it lives
only on the box. A new environment variable has to be added there by hand; deploying will not
carry it up.

### What the smoke test covers

Health, the NotoSans font files **inside the container**, `S3_BUCKET_NAME`, `BREVO_API_KEY`, and
whether the container is crash-looping.

The font check runs inside the container deliberately. The rate-card and GST-invoice outage of
September 2026 was a packaging bug: the `.ttf` files were present in the repo the entire time and
absent from the image, because the runtime stage copied `dist` and `prisma` but not `assets`.
`@react-pdf/renderer` reads a registered font at render time and throws `ENOENT`, so every rate
card and every invoice returned a 500 while everything passed locally. Any check run outside the
container would have stayed green throughout.
