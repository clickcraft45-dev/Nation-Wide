...
function PageNumeral({ items }: { items: NavItem[] }) {
  ...
  return (
    <span
      aria-hidden
      className="pointer-events-none absolute -bottom-6 right-4 -z-10 select-none font-semibold leading-none text-transparent text-[8rem] [-webkit-text-stroke:3px_var(--page-numeral-stroke)] lg:text-[13rem] lg:[-webkit-text-stroke:4.5px_var(--page-numeral-stroke)]"
    >
...
export function DashboardShell({
  ...
  return (
    <div className="app-ambient fixed inset-0 flex overflow-hidden">
      <Sidebar items={items} groups={groups} />
      <div className="relative isolate flex min-w-0 flex-1 flex-col">
        <PageNumeral items={items} />
        <Topbar user={user} items={items} profileHref={profileHref} />
