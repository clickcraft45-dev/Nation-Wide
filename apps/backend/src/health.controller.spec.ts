...
// Prisma's MongoDB server-selection timeout is 30s and an unreachable Redis is retried for ~10s —
// so /health used to sit open past both. An orchestrator whose probe times out kills the container
...
function harness(
  ...
  const controller = new HealthController(
    { $runCommandRaw: jest.fn(database) } as unknown as PrismaService,
