...
export class RateCardDocumentsService {
  ...
  async generate(
    ...
    const created = await this.prisma.rateCardDocument.create({
      data: {
        ...
        templateKey,
        version,
        // Prisma's Bytes field wants a plain ArrayBuffer-backed Uint8Array, not the exact
        // Buffer<ArrayBufferLike> type — Uint8Array.from copies into a fresh, correctly-typed one.
  ...
  async getPdf(
    ...
    return { document: doc, pdf: Buffer.from(doc.pdf) };
  }
