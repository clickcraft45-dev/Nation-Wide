...
// storage/ writes are the one side effect these tests don't want; the PDF renderer is mocked, so
// the only real file operation left is the write, stubbed here.
...
describe('InvoicesService', () => {
  ...
  describe('issuing', () => {
    ...
    it('numbers the invoice per financial year', async () => {
      ...
      const counterName = (
        prisma.$runCommandRaw.mock.calls[0][0] as { query: { _id: string } }
      ).query._id;
