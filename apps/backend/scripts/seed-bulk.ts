...
function queueInvoice(
  ...
  pending.push({
    ...
    data: {
      ...
      // No PDF: rendering one file per invoice would add gigabytes to storage/ for demo data.
...
async function dbSizeMb() {
  const stats = (await prisma.$runCommandRaw({ dbStats: 1, scale: 1 })) as any;
  const mb = (n: unknown) => Number(n) / 1_048_576;
  return {
    data: mb(stats.dataSize),
    storage: mb(stats.storageSize),
    index: mb(stats.indexSize),
