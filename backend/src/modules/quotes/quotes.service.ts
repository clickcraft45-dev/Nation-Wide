...
export class QuotesService {
  ...
  async create(
    ...
    if (status !== 'NEEDS_MANUAL_REVIEW') {
      ...
      if (computedOptions.length > 0) {
        ...
        // No rate card covers this destination/weight, so there is no price to show. An admin
        // prices it first (setManualQuote -> QUOTED -> customer accepts); only then does the
        // pickup request go out to partners.
    ...
    try {
      const quote = await this.prisma.quote.create({
        data: {
          ...
          // The customer may leave the recipient for the pickup partner to take down at the door.
          destName: dto.destination.name || null,
          destPhone: dto.destination.phone || null,
          destAddressLine1: dto.destination.addressLine1 || null,
          destAddressLine2: dto.destination.addressLine2 || null,
          destCity: dto.destination.city || null,
          destState: dto.destination.state || null,
          destPostalCode: dto.destination.postalCode || null,
  ...
  async preview(dto: QuotePreviewQueryDto): Promise<QuotePreviewResultDto> {
    ...
    if (computedOptions.length === 0) {
      // Mirrors create() exactly — see the note there.
      return {
        status: 'NEEDS_MANUAL_REVIEW',
        reviewReason: 'NO_RATE_AVAILABLE',
        options: [],
