...
export class InvoicesService {
  ...
  /**
   * Renders and uploads the PDF once, at issue time. Shared by both issue paths.
   *
   * Returns the S3 object key, which is what Invoice.pdfPath stores. Partitioned by the
   * invoice's own date rather than upload time so a back-dated re-issue lands in the period it
   * belongs to, and so a year's invoices can be listed under one prefix.
   */
  private async storePdf(
    ...
    const key = this.storageKeyFor(invoice);
    await this.storage.put(key, buffer, 'application/pdf');
    return key;
  }

  /** e.g. "invoices/2026/09/NW-2026-27-00042.pdf" — slashes are illegal inside a key segment. */
  private storageKeyFor(invoice: Invoice): string {
    const date = invoice.invoiceDate ?? new Date();
    const year = date.getUTCFullYear();
    const month = String(date.getUTCMonth() + 1).padStart(2, '0');
    return `invoices/${year}/${month}/${this.filenameFor(invoice)}`;
  }
