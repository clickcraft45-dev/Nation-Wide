...
class EnvironmentVariables {
  ...
  // ---------------------------------------------------------------------------
  // S3 — the permanent store for invoice PDFs, company logos and rate-card PDFs.
  // ---------------------------------------------------------------------------
  // There is deliberately NO AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY here. On EC2 the SDK
  // reads the instance IAM role through its default provider chain; adding static keys to this
  // list is what would make committing one possible, so the option does not exist.
  @IsOptional()
  @IsString()
  AWS_REGION?: string;

  // Optional so the app still boots (and /health still answers) with storage unconfigured —
  // StorageService throws a named error on first use instead, which is far easier to diagnose
  // than a boot loop. Every deployment that issues invoices must set it.
  @IsOptional()
  @IsString()
